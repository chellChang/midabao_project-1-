...
@property (nonatomic,strong)UIView *bottomlineSuperView;
@property (nonatomic,strong)UIView *bottomlineSubView;
...
- (UIView *)segmentView {
    if(!_segmentView) {
        ...
        _bottomlineSuperView=[[UIView alloc]initWithFrame:CGRectMake(0, self.segmentBarHeight-2, [UIScreen mainScreen].bounds.size.width/self.segmentButtons.count, 2)];
        
        _bottomlineSuperView.backgroundColor=[UIColor clearColor];
        _bottomlineSubView=[[UIView alloc]initWithFrame:CGRectMake(_bottomlineSuperView.frame.size.width/2.0-16, 0, 32, 2)];
        _bottomlineSubView.backgroundColor=RGB(0x3379EA);
        [_bottomlineSuperView addSubview:_bottomlineSubView];
        [_segmentView addSubview:_bottomlineSuperView];
...
- (void)configureSegmentButtonLayout {
    if([self.segmentButtons count] > 0) {
        ...
        [_segmentView bringSubviewToFront:_bottomlineSuperView];
    }
}
...
- (void)segmentButtonEvent:(UIButton *)segmentButton {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        if (clickIndex!=0) {
            self.bottomlineSubView.frame=CGRectMake(self.bottomlineSuperView.frame.size.width/2.0-24, 0, 48, 2);
        }else{
            self.bottomlineSubView.frame=CGRectMake(self.bottomlineSuperView.frame.size.width/2.0-16, 0, 32, 2);
        }
        self.bottomlineSuperView.frame=CGRectMake(clickIndex*[UIScreen mainScreen].bounds.size.width/self.segmentButtons.count, self.segmentBarHeight-2, [UIScreen mainScreen].bounds.size.width/self.segmentButtons.count, 2);
    }];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if(context == &HHHorizontalPagingViewPanContext) {
        ...
        NSLog(@"---headerDisplayHeight:%f",headerDisplayHeight);
...
-(void)setTopReirshUI{
//    [self.currentScrollView setContentOffset:CGPointMake(self.currentScrollView.contentOffset.x, -264) animated:YES];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.bottomlineSuperView.frame=CGRectMake(currentPage*[UIScreen mainScreen].bounds.size.width/self.segmentButtons.count, self.segmentBarHeight-2, [UIScreen mainScreen].bounds.size.width/self.segmentButtons.count, 2);
        if (currentPage!=0) {
            self.bottomlineSubView.frame=CGRectMake(self.bottomlineSuperView.frame.size.width/2.0-24, 0, 48, 2);
        }else{
            self.bottomlineSubView.frame=CGRectMake(self.bottomlineSuperView.frame.size.width/2.0-16, 0, 32, 2);
