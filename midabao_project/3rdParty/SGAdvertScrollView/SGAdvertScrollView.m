...
- (void)layoutSubviews {
    ...
    self.topLabel.frame = CGRectMake(topLabelX+5, topLabelY, topLabelW-5, topLabelH);
    ...
    self.bottomLabel.frame = CGRectMake(bottomLabelX+5, bottomLabelY, bottomLabelW-5, bottomLabelH);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.advertScrollViewStyle == SGAdvertScrollViewStyleMore) {
        ...
        if (self.imageArr.count>0) {
            ...
            cell.topSignImageView.image = [UIImage imageNamed:self.dotimgname];
        }
        if (indexPath.item==self.titleArr.count) {
            cell.topLabel.text=@"";
        }else{
        ...
        if (self.bottomImageArr.count>0) {
            ...
            cell.bottomSignImageView.image = [UIImage imageNamed:self.dotimgname];
        }

        if (indexPath.item==self.bottomTitleArr.count) {
            cell.bottomLabel.text = @"";
            cell.bottomSignImageView.hidden=YES;
        }else{
            cell.bottomLabel.text = self.bottomTitleArr[indexPath.item];
            cell.bottomSignImageView.hidden=NO;
        }
