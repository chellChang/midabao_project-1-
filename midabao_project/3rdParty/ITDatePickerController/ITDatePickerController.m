...
static NSInteger kITDatePickerViewHeight = 166+48;
...
-(void)datePickerControllerClcikLeftTabItem{
    if ([self.delegate respondsToSelector:@selector(datePickerControllerClcikLeftTabItem)]) {
        [self.delegate datePickerControllerClcikLeftTabItem];
    }
}
...
//    self.maximumDate = [self.dateFormatter dateFromString:@"2018.10"];
    self.maximumDate = [NSDate date];//最大时间设置当前时间
    self.minimumDate = [self.dateFormatter dateFromString:@"2017.10"];
    
    _showToday = YES;
   
    self.defaultDate = [NSDate date];
    if (self.minimumYear!=self.maximumYear) {
        if (self.thisYear==self.minimumYear) {
            self.monthCount=12-self.minimumMonth+1;
        }else if(self.thisYear==self.maximumYear){
            self.monthCount=self.maximumMonth;
        }else{
            self.monthCount=12;
        }
        
    }else{
        self.monthCount=self.maximumMonth-self.minimumMonth+1;
    }
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (component==0) {
        return self.maximumYear-self.minimumYear+1;
    }else{
...
//            text = @"至今";
        ...
        if (self.selectedYear==self.minimumYear) {
            NSInteger index = row % self.monthCount;
            //        NSInteger index = self.minimumMonth+1+row % self.monthCount;
            if (self.isHideMonth &&
                self.minimumMonth+index == self.thisMonth &&
                self.selectedMonth == self.thisMonth &&
                self.selectedYear == self.thisYear) {
                return @"本月";
            }
            return [NSString stringWithFormat:@"%02ld", self.minimumMonth+index];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if (self.minimumYear!=self.maximumYear) {
            if (self.selectedYear==self.minimumYear) {
                self.monthCount=12-self.minimumMonth+1;
            }else if(self.selectedYear==self.maximumYear){
                self.monthCount=self.maximumMonth;
            }else{
                self.monthCount=12;
            }
            ...
            self.monthCount=self.maximumMonth-self.minimumMonth;
        }
        ...
        if (self.minimumYear!=self.maximumYear) {
            if (self.selectedYear==self.minimumYear) {
                self.selectedMonth=self.minimumMonth+row;
            }else{
...
//    NSInteger selctRow = (maxCount/2)/self.yearCount*self.yearCount+ self.defaultYear- self.minimumYear;
    NSInteger selctRow  = self.defaultYear-self.minimumYear;
...
//    NSInteger selectRow = (maxCount/2)/self.monthCount*self.monthCount+ self.defaultMonth-1;
    if (self.defaultYear==self.minimumYear) {
        selectRow = self.defaultMonth-self.minimumMonth;
    }else{
        selectRow = self.defaultMonth-1;
    }
...
- (UIBarButtonItem *)cancelButton {
    if (!_cancelButton) {
        ...
        [cancelBtn setTitle:@"全部" forState:UIControlStateNormal];
        [cancelBtn setTitleColor:RGB(0x3379EA) forState:UIControlStateNormal];
        cancelBtn.frame=CGRectMake(0, 0, 50, 48);
        [cancelBtn addTarget:self action:@selector(cancelButtonOnClicked) forControlEvents:UIControlEventTouchUpInside];
        _cancelButton = [[UIBarButtonItem alloc]initWithCustomView:cancelBtn];
    }
...
- (UIBarButtonItem *)confirmButton {
    if (!_confirmButton) {
        ...
        [okBtn setTitleColor:RGB(0x3379EA) forState:UIControlStateNormal];
        okBtn.frame=CGRectMake(0, 0, 50, 48);
        [okBtn addTarget:self action:@selector(confirmButtonOnClicked) forControlEvents:UIControlEventTouchUpInside];
        _confirmButton =[[UIBarButtonItem alloc]initWithCustomView:okBtn];
...
- (UIView *)line {
    if (!_line) {
        _line = [[UIView alloc] initWithFrame:CGRectMake(0, 47.5, kITScreenWidth, 0.5)];
...
- (UIToolbar *)toolBar {
    if (!_toolBar) {
        _toolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, kITScreenWidth, 48)];
...
- (UIPickerView *)pickerView {
    if (!_pickerView) {
        _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 48, kITScreenWidth, 166)];
