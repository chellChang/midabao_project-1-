        ...
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            if (self.pageCount>1) {
                for (int i = (int)self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                    ...
                    CGRect originCellFrame = CGRectMake(_pageSize.width * i, 0, _pageSize.width-10, _pageSize.height);//如果没有缩小效果的情况下的本该的Frame
                    ...
                    if (delta < _pageSize.width) {
                        ...
                        cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, 0, inset, inset));
                ...
                PGIndexBannerSubiew *cell = [_cells objectAtIndex:0];
                CGFloat origin = cell.frame.origin.x;
                CGFloat delta = fabs(origin - offset);
                
                CGRect originCellFrame = CGRectMake(5, 0, _pageSize.width-10, _pageSize.height);//如果没有缩小效果的情况下的本该的Frame
                cell.coverView.alpha = (delta / _pageSize.width) * (1 - _minimumPageAlpha);
